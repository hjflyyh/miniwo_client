import { _decorator, Button, Component, director, dragonBones, KeyCode, Label, Node, resources, RichText, Sprite, SpriteFrame, TiledLayer, Tween, tween, UIOpacity, UITransform, v2, v3, Vec2, Vec3 } from 'cc';
import { EventType } from 'db://assets/src/EventType';
// import { observer } from 'db://assets/src/game/App';
// import { GlobalConfig } from 'db://assets/src/game/config/GlobalConfig';
// import { JietuComponent } from 'db://assets/src/manager/JietuComponent';
// import { PlayerManager } from 'db://assets/src/NPC/PlayerManager';
// import { NpcName, FeelingImgUrl } from 'db://assets/src/StaticUtils/NPCConfig';
...
export class NpcController extends Component {
    ...
    protected onLoad(): void {
        this.clickNode = this.node.getChildByName("clickNode").getComponent(Button);
    ...
    update(deltaTime: number) {
        if (this._initSuccess) {
            ...
            this.checkSpeakObj();
            let mapScript = director.getScene().getComponentInChildren(MapGameScene).getMapScript();
    ...
    serverPathMove(path: any, direction = "down") {
        ...
        for (let i = 1; i < path.length; i++) {
            let actionPosition = this.mapEditor.getGridToPosition(v2(path[i].x, path[i].y));
            sequence.push(tween(this.node)
                .to(0.18, { position: new Vec3(actionPosition.x, actionPosition.y) }).call(() => {
                    ...
                    this._curTile = this.mapEditor.getPositionToGrid(new Vec2(this.node.position.x, this.node.position.y));
    ...
    _getTilePos(posInPixel: { x: number, y: number }) {
        const mapSize = this.mapEditor.getComponent(UITransform).contentSize;
    ...
    checkMorpheusCapture(content) {
        ...
        let gameScene = director.getScene().getComponentInChildren(MapGameScene)
    ...
    setNpcConfig(npcId: string, houseId: string, name: string, intro: string) {
        this.npcConfig.houseId = houseId;
        this.npcConfig.npcId = npcId;
        this.npcConfig.name = name;
        this.npcConfig.npcIntro = intro;
    }

    onClickShowNpcDesginDialog() {
        this.mapEditor.showNpcDesignDialog(this.npcConfig);
    }
}
